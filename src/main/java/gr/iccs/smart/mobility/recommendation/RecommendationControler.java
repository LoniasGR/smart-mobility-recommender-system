...
public class RecommendationControler {
    ...
    public FeatureCollection suggestRoute(@PathVariable String username, @RequestBody UserRouteDTO route) {
        var user = userService.getById(username);
        var start = route.startingLocation().toPoint();
        var finish = route.endingLocation().toPoint();
        return recommendationService.recommendationV2(start, finish, user);
    }
}
